---
title: "Crime and Communities Exercise"
...
for (a in log10(seq(1,10,1))) {
  ...
  cat("\nTrain correlation coefficient: ", cor(as.matrix(df.train$ViolentCrimesPerPop),       
                                             fitted.elastic.train)[1])
  cat("\nTest correlation coefficient: ", cor(as.matrix(df.test$ViolentCrimesPerPop), 
                                            fitted.elastic.test)[1])
}
```

Elastic net performs the best (looking at the test correlation coefficient), but not by much. This is unsurprising, since elastic net is a superset of the other models. Picking the optimal value from above, $\alpha = 0.30103$.
